...
def prompt_bool(prompt_text: str, default: bool = False) -> bool:
    """Versión segura para ejecuciones desatendidas (nohup)."""
    try:
        response = prompt(prompt_text).strip().lower()
        if not response:
            return default
        
        # Usamos startswith en lugar de índices [0] para evitar el error de rango
        ...
        # Si falla cualquier cosa en el prompt (como en nohup), devuelve el default
        return default
