...
class ConversationManager:
    ...
    AGENT_SYSTEM_PROMPT_FORMAT: ClassVar[str] = dedent(
        ...
        - Do not concede your position unless presented with a logically superior argument that survives your character's specific ethical scrutiny.
    ...
    MODERATOR_SYSTEM_PROMPT: ClassVar[str] = dedent(
        ...
        - If tension is building, select someone who will either escalate meaningfully or provide resolution
          based on their nature
        - When conversations become one-sided, bring in contrasting perspectives from characters with opposing traits
        - If a topic is exhausted, choose speakers whose personality makes them likely to introduce fresh but relevant
          angles
    ...
    def run_conversation(self) -> Iterator[tuple[str, Iterator[str]]]:
        ...
        while True:
            ...
            def parse_partial_json(json_string):
                # Si el texto está vacío, devolvemos un diccionario vacío en lugar de romper
                if not json_string or not json_string.strip():
                    return {}
                
                try:
                    from partial_json_parser import ensure_json
                    return json.loads(ensure_json(json_string))
                except Exception:
                    # Si la librería falla intentando leer, devolvemos vacío y seguimos
                    return {}
